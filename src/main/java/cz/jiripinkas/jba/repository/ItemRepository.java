...
public interface ItemRepository extends JpaRepository<Item, Integer> {
	...
	@Query("select sum(coalesce(i.likeCount, 0) + ((log(coalesce(i.clickCount, 0) + 1) * 10) + (log(coalesce(i.twitterRetweetCount, 0) + 1) * 10) + (log(coalesce(i.facebookShareCount, 0) + 1) * 10) + (log(coalesce(i.linkedinShareCount, 0) + 1) * 10))) / coalesce(count(i), 1) from Item i where i.blog.id = ?1 and i.savedDate > ?2")
